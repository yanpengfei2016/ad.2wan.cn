...
$table_query = mysqli_query($polardb_link, "SELECT code FROM sys_table_code WHERE `key` = '5w_tkio_click' LIMIT 1");
...
if(empty($table_name)){
    $table_name = '5w_tkio_click_fallback';
}

$insert_sql_pre = "INSERT INTO {$table_name} (game_id,ip,promote_id,promote_account,ctime,user_agent,ua_md5) VALUES"; # look here
...
function consume_queue($redis, $polardb_link){
    ...
    while (true){
        $data = $redis_link->lpop('ad_5tkio_queue'); # look here
        $sql = trim($data);
        if(!empty($sql)){
            $sql_arr[] = $sql;
        }else{
    ...
    if(!empty($sql_arr)){
        ...
        // var_dump("redis = {$redis_info} handle " . count($sql_arr));
        // var_dump($sql);
...
function insert_db($link, $sql){
    ...
    if(empty($result)){
        // var_dump(date('Y-m-d H:i:s') . " [ERROR] ad_5tkio_queue_consumer SQL query failed : ".mysqli_error($link));
        insert_db_retry($sql);
        exit;
    }
}
...
function insert_db_retry($sql){
    ...
    if(empty($result)){
        var_dump($sql);
        var_dump(date('Y-m-d H:i:s') . " [ERROR] retry ad_5tkio_queue_consumer SQL query failed : ".mysqli_error($polardb_link));
        exit;
    }
}
