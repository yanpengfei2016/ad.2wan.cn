...
$table_query = mysqli_query($polardb_link, "SELECT code FROM sys_table_code WHERE `key` = 'tkio_click' LIMIT 1");
...
if(empty($table_name)){
    $table_name = 'tkio_click_fallback';
}
...
function consume_queue($redis, $polardb_link){
    ...
    while (true){
        $data = $redis_link->lpop('ad_tkio_queue'); # look here
        $sql = trim($data);
        if(!empty($sql)){
            $sql_arr[] = $sql;
        }else{
...
function insert_db($link, $sql){
    ...
    if(empty($result)){
        // var_dump(date('Y-m-d H:i:s') . " [ERROR] ad_tkio_queue_consumer SQL query failed : ".mysqli_error($link));
        insert_db_retry($sql);
        exit;
    }
}
...
function insert_db_retry($sql){
    ...
    if(empty($result)){
        var_dump($sql);
        var_dump(date('Y-m-d H:i:s') . " [ERROR] retry ad_tkio_queue_consumer SQL query failed : ".mysqli_error($polardb_link));
        exit;
    }
}