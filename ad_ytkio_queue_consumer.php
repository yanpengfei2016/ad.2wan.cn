...
$table_query = mysqli_query($polardb_link, "SELECT code FROM sys_table_code WHERE `key` = 'yyx_tkio_click' LIMIT 1");
...
if(empty($table_name)){
    $table_name = 'yyx_tkio_click_fallback';
}
...
function consume_queue($redis, $polardb_link){
    ...
    while (true){
        $data = $redis_link->lpop('ad_ytkio_queue'); # look here
        $sql = trim($data);
        if(!empty($sql)){
            $sql_arr[] = $sql;
        }else{
...
function insert_db_retry($sql){
    ...
    if(empty($result)){
        var_dump($sql);
        var_dump(date('Y-m-d H:i:s') . " [ERROR] retry ad_ytkio_queue_consumer SQL query failed : ".mysqli_error($polardb_link));
        exit;
    }
}